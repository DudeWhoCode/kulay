...
func init() {
	//redisq.SetLogger(log.New(os.Stderr, "redisq: ", log.LstdFlags|log.Lshortfile))
}
...
var (
	redisServerBin, _  = filepath.Abs(filepath.Join("testdata", "redisq", "src", "redisq-server"))
	redisServerConf, _ = filepath.Abs(filepath.Join("testdata", "redisq.conf"))
)
...
func startRedis(port string, args ...string) (*redisProcess, error) {
	...
	baseArgs := []string{filepath.Join(dir, "redisq.conf"), "--port", port, "--dir", dir}
