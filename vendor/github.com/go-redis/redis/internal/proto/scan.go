...
func Scan(b []byte, v interface{}) error {
	switch v := v.(type) {
	case nil:
		return internal.RedisError("redisq: Scan(nil)")
	case *string:
	...
	default:
		return fmt.Errorf(
			"redisq: can't unmarshal %T (consider implementing BinaryUnmarshaler)", v)
	}
}
...
func ScanSlice(data []string, slice interface{}) error {
	...
	if !v.IsValid() {
		return fmt.Errorf("redisq: ScanSlice(nil)")
	}
	if v.Kind() != reflect.Ptr {
		return fmt.Errorf("redisq: ScanSlice(non-pointer %T)", slice)
	}
	v = v.Elem()
	if v.Kind() != reflect.Slice {
		return fmt.Errorf("redisq: ScanSlice(non-slice %T)", slice)
	}

	for i, s := range data {
		elem := internal.SliceNextElem(v)
		if err := Scan(internal.StringToBytes(s), elem.Addr().Interface()); err != nil {
			return fmt.Errorf("redisq: ScanSlice(index=%d value=%q) failed: %s", i, s, err)
		}
	}
