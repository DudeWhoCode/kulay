...
func (p *Reader) ReadLine() ([]byte, error) {
	...
	if len(line) == 0 {
		return nil, internal.RedisError("redisq: reply is empty")
	}
	if isNilReply(line) {
...
func (p *Reader) ReadReply(m MultiBulkParse) (interface{}, error) {
	...
	return nil, fmt.Errorf("redisq: can't parse %.100q", line)
}
...
func (p *Reader) ReadIntReply() (int64, error) {
	...
	case IntReply:
		return parseInt(line[1:], 10, 64)
	default:
		return 0, fmt.Errorf("redisq: can't parse int reply: %.100q", line)
	}
}
...
func (p *Reader) ReadTmpBytesReply() ([]byte, error) {
	...
	case StatusReply:
		return parseStatusValue(line), nil
	default:
		return nil, fmt.Errorf("redisq: can't parse string reply: %.100q", line)
	}
}
...
func (p *Reader) ReadArrayReply(m MultiBulkParse) (interface{}, error) {
	...
	case ArrayReply:
		...
		return m(p, n)
	default:
		return nil, fmt.Errorf("redisq: can't parse array reply: %.100q", line)
	}
}
...
func (p *Reader) ReadArrayLen() (int64, error) {
	...
	default:
		return 0, fmt.Errorf("redisq: can't parse array reply: %.100q", line)
	}
}
...
func (p *Reader) ReadScanReply() ([]string, uint64, error) {
	...
	if n != 2 {
		return nil, 0, fmt.Errorf("redisq: got %d elements in scan reply, expected 2", n)
	}
